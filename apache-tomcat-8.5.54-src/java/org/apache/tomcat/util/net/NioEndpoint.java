...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel> {
    ...
    public void startInternal() throws Exception {
        ...
        if (!running) {
            running = true;
            paused = false;
            // 用于SocketProcessor对象的存储
            processorCache = new SynchronizedStack<>(SynchronizedStack.DEFAULT_SIZE,
                    socketProperties.getProcessorCache());
            // 为轮询器时间缓存PollerEvent
            eventCache = new SynchronizedStack<>(SynchronizedStack.DEFAULT_SIZE,
                            socketProperties.getEventCache());
            // Bytebuffer缓存,每个通道持有一组缓冲区(2个,SSL持有4个)
    ...
    public class Poller implements Runnable {
        ...
        public boolean events() {
            ...
            for (int i = 0, size = events.size(); i < size && (pe = events.poll()) != null; i++ ) {
                result = true;
                try {
                    // PollerEvent实现了Runnable接口，这里直接调用了
                    pe.run();
                    // 重置
                    pe.reset();
                    if (running && !paused) {
         ...
         *
         * 后台线程将socket添加到poller,poller检查触发时间，并在事件发生时将关联的socket交给适当的处理器
         */
        @Override
        public void run() {
                ...
                try {
                    ...
                    if (!close) {
                        ...
                        if (wakeupCounter.getAndSet(-1) > 0) {
                            ...
                            // 阻塞，知道至少有一个通道在注册的事件上就绪了，最长阻塞时间为selectorTimeout，单位毫秒
        ...
        protected void processKey(SelectionKey sk, NioSocketWrapper attachment) {
            try {
                if ( close ) {
                    // 取消 SelectionKey
                    cancelledKey(sk);
                } else if ( sk.isValid() && attachment != null ) { // 如果有效且有数据

                    if (sk.isReadable() || sk.isWritable() ) {
                        if ( attachment.getSendfileData() != null ) {// 附件数据不是null
